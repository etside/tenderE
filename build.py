...
data = json.loads(pathlib.Path("tenders.json").read_text())

CATEGORIES = ["All", "Software & Web", "AI & Data", "Consulting", "ICT Infrastructure", "ICT Services"]

js = textwrap.dedent(f"""\
const TENDERS = {json.dumps(data, ensure_ascii=False, indent=2)};
...
pathlib.Path("data.js").write_text(js)
print(f"Built data.js with {len(data)} tenders.")
